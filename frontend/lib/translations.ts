export type Lang = "es" | "en";

export const t = {
  es: {
    nav: {
      services: "Servicios",
      howIWork: "Cómo trabajo",
      projects: "Proyectos",
      about: "Sobre mí",
      contact: "Contacto",
      blog: "Blog",
      cta: "Hablemos",
    },
    hero: {
      badge: "Analista de Datos · Business Intelligence",
      headline1: "Tus datos ya existen.",
      headline2: "Lo que falta es convertirlos en información.",
      sub: "Cada venta, cada compra, cada movimiento de tu negocio deja un rastro. Lo leo y te entrego información clara para que vos tomes las decisiones.",
      cta1: "Hablemos de tu negocio",
      cta2: "Ver servicios",
      stat1: { value: "12+", label: "años en retail y consumo masivo" },
      stat2: { value: "4", label: "empresas líderes analizadas" },
      stat3: { value: "100%", label: "confidencialidad garantizada" },
    },
    problem: {
      tag: "El diagnóstico",
      title: "Datos tenés. Información es lo que falta.",
      subtitle:
        "Fechas, números, ventas, movimientos — todo está guardado en algún lado. El problema no es la cantidad: es que sin el análisis correcto, los datos no dicen nada. La información sí.",
      cards: [
        {
          icon: "TrendingDown",
          title: "Precios que sangran margen",
          desc: "Vendés mucho pero ganás poco. Sin análisis de costos y márgenes, es imposible saber dónde perdés rentabilidad.",
        },
        {
          icon: "Package",
          title: "Inventario que duerme o falta",
          desc: "Stock inmovilizado que cuesta dinero, o quiebres que te hacen perder ventas. Ambos tienen solución con datos.",
        },
        {
          icon: "BarChart2",
          title: "KPIs que nadie entiende",
          desc: "Reportes llenos de números sin contexto. Sabés que algo está mal, pero no exactamente qué ni por qué.",
        },
        {
          icon: "Clock",
          title: "Tiempo perdido en planillas",
          desc: "Horas armando reportes a mano que podrían automatizarse. Ese tiempo vale — y se puede recuperar.",
        },
      ],
    },
    services: {
      tag: "Lo que hago",
      title: "De datos a información accionable",
      subtitle:
        "Leo lo que ya registrás y te entrego información concreta: qué está pasando y por qué. La decisión es tuya.",
      items: [
        {
          icon: "DollarSign",
          title: "Análisis de Precios y Márgenes",
          desc: "Modelos de costos, rentabilidad por producto y simulación de promociones para que cada decisión comercial esté respaldada.",
          tags: ["Excel Avanzado", "Power BI"],
        },
        {
          icon: "LayoutDashboard",
          title: "Dashboards y KPIs",
          desc: "Tableros visuales que muestran lo que realmente importa de tu negocio, actualizados y fáciles de leer.",
          tags: ["Power BI", "Looker Studio"],
        },
        {
          icon: "RefreshCw",
          title: "Automatización de Reportes",
          desc: "Convertí horas de trabajo manual en procesos automáticos. Más tiempo para tu negocio, menos para planillas.",
          tags: ["Python", "Power Query"],
        },
        {
          icon: "Package",
          title: "Análisis de Inventario",
          desc: "Rotación de productos, vencimientos, stock óptimo. Tomá decisiones de compra con evidencia, no con intuición.",
          tags: ["SQL", "Excel"],
        },
        {
          icon: "Database",
          title: "Organización de Datos",
          desc: "Tus datos dispersos, limpios y estructurados. Una base sólida para cualquier análisis futuro.",
          tags: ["SQL", "Python", "ETL"],
        },
        {
          icon: "MessageSquare",
          title: "Consultoría a Medida",
          desc: "Si tu problema no encaja en un cajón estándar, trabajamos juntos desde el diagnóstico hasta la solución.",
          tags: ["A medida"],
        },
      ],
    },
    howIWork: {
      tag: "Mi forma de trabajar",
      title: "Tus datos hablan. Yo los interpreto.",
      subtitle:
        "No soy una agencia. Soy un analista comprometido con el crecimiento de tu negocio.",
      steps: [
        {
          num: "01",
          title: "Escucho antes de analizar",
          desc: "Primero entiendo tu negocio, tus decisiones y tus dolores. Los datos vienen después.",
        },
        {
          num: "02",
          title: "Trabajo con lo que tenés",
          desc: "No necesitás sistemas caros ni grandes cambios. Empiezo con tus planillas, tu sistema y tus datos actuales.",
        },
        {
          num: "03",
          title: "Explico sin jerga",
          desc: "Cada análisis viene con una conclusión clara: qué significa, qué podés hacer y qué impacto tiene.",
        },
        {
          num: "04",
          title: "Estoy disponible cuando me necesitás",
          desc: "No entrego un informe y desaparezco. Soy tu respaldo continuo para interpretar lo que pasa en tu negocio.",
        },
      ],
      values: [
        { icon: "Shield", label: "Confidencialidad total" },
        { icon: "Handshake", label: "Compromiso real" },
        { icon: "Clock", label: "Disponibilidad y respuesta" },
        { icon: "Heart", label: "Vocación de servicio" },
      ],
    },
    projects: {
      tag: "Proyectos",
      title: "Análisis reales, resultados concretos",
      subtitle: "Una muestra de trabajos donde los datos generaron valor real.",
      items: [
        {
          title: "Análisis de Costos y Pricing Comercial",
          company: "Empresa líder en consumo masivo · 2026",
          desc: "Modelos en Excel avanzado para evaluar estructura de costos, márgenes y acciones promocionales. KPIs y dashboards en Power BI para seguimiento del desempeño comercial.",
          tags: ["Excel Avanzado", "Power BI", "Pricing"],
          highlight: "Mejor visibilidad de márgenes por categoría",
        },
        {
          title: "Análisis de Acceso a Internet por Provincia",
          company: "Henry Bootcamp · 2024",
          desc: "EDA y limpieza de datasets de conectividad nacional. Dashboards interactivos en Power BI para identificar brechas y oportunidades de expansión por provincia.",
          tags: ["Python", "Pandas", "Power BI"],
          highlight: "Identificación de 8 provincias con brecha crítica",
          link: "https://github.com/moreiracristian/Henry-PIDA-CM",
        },
        {
          title: "MVP para Uber — Análisis Medioambiental",
          company: "Henry Bootcamp · 2024",
          desc: "Análisis de variables medioambientales que impactan la operación de Uber en Nueva York. Propuesta de adopción de vehículos eléctricos con pipeline ETL completo.",
          tags: ["Python", "Airflow", "MySQL", "Streamlit"],
          highlight: "Proyección de 40% de reducción en costos operativos",
          link: "https://github.com/moreiracristian/Henry-Arcope-PF",
        },
        {
          title: "Sistema de Recomendación de Películas",
          company: "Henry Bootcamp · 2024",
          desc: "Motor de recomendación por similitud de contenido con API en FastAPI para consultas en tiempo real. Machine learning aplicado a datos reales de IMDB.",
          tags: ["Scikit-learn", "FastAPI", "Python"],
          highlight: "API REST con respuesta en tiempo real",
          link: "https://github.com/moreiracristian/Henry-PIMLOps-CM",
        },
      ],
    },
    about: {
      tag: "Sobre mí",
      title: "12 años mirando números que mueven negocios",
      paragraphs: [
        "Trabajé más de una década en las empresas de retail más grandes de Argentina — Falabella, Cencosud, Sodimac, Cetrogar. Vi de cerca cómo las decisiones basadas en datos cambian el resultado de un negocio — y también cómo la falta de análisis los hunde.",
        "En esos años aprendí que los problemas de datos son siempre problemas de negocio: precios que erosionan el margen sin que nadie lo note, stock que se inmoviliza en el depósito, reportes que se arman pero nadie lee. Entiendo ese lenguaje y sé traducirlo en decisiones concretas.",
        "Hoy acompaño a PyMEs que quieren crecer pero no tienen un analista propio. Soy tu respaldo de datos: comprometido, disponible y siempre en lenguaje de negocio.",
        "Formación en Data Science (Henry Bootcamp, +800 hs), Business Intelligence (UTN) e Ingeniería en Sistemas. Todo al servicio de un objetivo simple: que tus datos trabajen para vos.",
      ],
      skills: [
        "Excel Avanzado",
        "Power BI",
        "SQL",
        "Python",
        "Power Query",
        "Tableau",
        "Airflow",
        "PostgreSQL",
      ],
      experience: [
        { company: "Empresa líder en consumo masivo", role: "Analista de Compras", period: "2025 – Hoy" },
        { company: "CETROGAR S.A.", role: "Analista de Cobranzas", period: "2019 – 2025" },
        { company: "CENCOSUD S.A.", role: "Analista Admin. y Control de Gestión", period: "2013 – 2016" },
        { company: "FALABELLA S.A.", role: "Analista de Tesorería Corporativa", period: "2010 – 2013" },
      ],
    },
    contact: {
      tag: "Contacto",
      title: "Contame de tu negocio",
      subtitle:
        "Toda consulta es confidencial. Sin compromiso. Respondiendo en menos de 24 horas.",
      form: {
        name: "Tu nombre",
        email: "Tu email",
        company: "Tu empresa / rubro",
        message: "¿Cuál es tu mayor desafío con los datos de tu negocio?",
        send: "Enviar mensaje",
        sending: "Enviando...",
        success: "¡Mensaje recibido! Te respondo en menos de 24 horas.",
        error: "Algo salió mal. Escribime directo a moreiracristian@2moreit.com",
      },
      direct: {
        title: "O escribime directamente",
        whatsapp: "WhatsApp",
        email: "Email",
        linkedin: "LinkedIn",
      },
    },
    newsletter: {
      tag: "Newsletter",
      title: "Un insight por semana para tu negocio",
      subtitle:
        "Sin spam. Sin jerga técnica. Solo una idea práctica cada semana para que uses tus datos mejor.",
      placeholder: "Tu email",
      cta: "Suscribirme",
      subscribing: "Suscribiendo...",
      success: "¡Suscripto! Te llega el primer insight esta semana.",
      error: "Algo salió mal. Intentá de nuevo.",
      privacy: "Sin spam. Podés darte de baja cuando quieras.",
    },
    segments: {
      tag: "¿Para quién trabajo?",
      title: "Tu negocio tiene datos. Los convierto en información.",
      subtitle: "Trabajo con empresas que quieren crecer pero no tienen un analista de datos propio.",
      items: [
        {
          icon: "ShoppingCart",
          title: "Retail y consumo masivo",
          desc: "Empresas con productos, ventas y costos que necesitan visibilidad de márgenes, rotación y performance comercial.",
          examples: ["Supermercados", "Distribuidoras", "Importadoras"],
        },
        {
          icon: "Truck",
          title: "Mayoristas y distribuidoras",
          desc: "Negocios con alta rotación de stock que necesitan controlar reposición, quiebres y rentabilidad por cliente.",
          examples: ["Mayoristas", "Logística", "Proveedores B2B"],
        },
        {
          icon: "Store",
          title: "Comercios y tiendas",
          desc: "Comercios que quieren saber qué productos vender, cuándo reponer y cómo maximizar el margen por metro cuadrado.",
          examples: ["Ferreterías", "Farmacias", "Indumentaria"],
        },
        {
          icon: "Building2",
          title: "Empresas sin equipo de datos",
          desc: "Cualquier PyME que toma decisiones importantes con planillas desactualizadas o sin métricas claras.",
          examples: ["Servicios", "Manufactura", "Agropecuario"],
        },
      ],
    },
    faq: {
      tag: "Preguntas frecuentes",
      title: "Lo que más me preguntan antes de empezar",
      subtitle: "Si tu duda no está acá, escribime. Toda consulta es bienvenida.",
      items: [
        {
          q: "¿Necesito un sistema especial para trabajar con vos?",
          a: "No. Trabajo con lo que ya tenés: Excel, planillas de Google, exportaciones de tu sistema de gestión, o incluso archivos manuales. No necesitás invertir en tecnología nueva para empezar.",
        },
        {
          q: "¿Cuánto tiempo lleva tener los primeros resultados?",
          a: "Depende del tipo de análisis, pero en general el primer informe está listo entre 5 y 10 días hábiles desde que recibo los datos. Los dashboards en Power BI pueden tardar un poco más según la complejidad.",
        },
        {
          q: "¿Mis datos están seguros y son confidenciales?",
          a: "Sí, totalmente. Firmamos un acuerdo de confidencialidad antes de compartir cualquier archivo. Nunca comparto ni uso tus datos para ningún otro propósito. Tu información es tuya.",
        },
        {
          q: "¿Qué diferencia hay entre los servicios habituales y los proyectos más complejos?",
          a: "Los servicios habituales resuelven problemas de visibilidad: dashboards, análisis de márgenes, automatización de reportes. Ves mejor lo que ya tenés. Los proyectos más complejos van un paso más allá: automatización de procesos completos, modelos que predicen comportamiento futuro, o infraestructura para manejar grandes volúmenes de datos. La diferencia no es de herramienta — es de profundidad y alcance del problema a resolver.",
        },
        {
          q: "¿Qué tipo de empresa es candidata para un proyecto más complejo?",
          a: "Empresas que ya tienen datos históricos acumulados, un volumen de operaciones significativo, y preguntas concretas que un dashboard estándar no puede responder. No hace falta tener un equipo de IT propio: hace falta tener el problema claro y los datos disponibles. En la primera charla evaluamos juntos si tiene sentido encararlo.",
        },
        {
          q: "¿Qué pasa si no sé bien qué necesito?",
          a: "Eso es exactamente para lo que sirve la primera conversación. Contame cómo tomás decisiones hoy, qué información te falta y qué problema te quita el sueño. A partir de eso definimos juntos qué tiene sentido analizar.",
        },
        {
          q: "¿Con qué tamaño de empresa trabajás?",
          a: "Con empresas de distintos tamaños y necesidades. Desde PyMEs que recién empiezan a organizar sus datos, hasta empresas medianas con operaciones más complejas que necesitan automatización, modelos predictivos o infraestructura de datos a escala. El tamaño no define si puedo ayudarte — lo define el tipo de problema que tenés.",
        },
      ],
    },
    techExpertise: {
      tag: "Para proyectos más complejos",
      title: "Cuando el negocio pide más que un reporte",
      subtitle: "Para empresas con más datos, más sistemas o más preguntas sin respuesta. Mismo compromiso, mayor profundidad.",
      cta: "Hablemos de tu proyecto",
      tracks: [
        {
          num: "01",
          icon: "BarChart3",
          title: "Business Intelligence & Dashboards",
          desc: "Consolidación de múltiples fuentes de datos en paneles visuales e interactivos que muestran el rendimiento del negocio en tiempo real.",
          modules: ["Power BI", "Tableau", "DAX avanzado", "Power Query", "Matplotlib / Seaborn"],
        },
        {
          num: "02",
          icon: "LineChart",
          title: "People Analytics",
          desc: "Analítica de datos aplicada a RR.HH.: métricas de contratación, análisis del mercado laboral y evaluación de rotación para optimizar la retención de talento.",
          modules: ["SQL", "Python", "KPIs de RRHH", "Estadística descriptiva", "Visualización"],
        },
        {
          num: "03",
          icon: "Code2",
          title: "Inteligencia Competitiva",
          desc: "Monitoreo automatizado del mercado y la competencia: rastreo de precios, ofertas o licitaciones para ajustar estrategias comerciales dinámicamente.",
          modules: ["Web scraping", "Python", "Automatización", "SQL", "Reportes automáticos"],
        },
        {
          num: "04",
          icon: "Brain",
          title: "Modelos Predictivos",
          desc: "Tu negocio anticipa en lugar de reaccionar. Predicción de demanda, detección de fuga de clientes y estimación de ventas futuras con datos históricos.",
          modules: ["Predicción de demanda", "Clasificación", "Series de tiempo", "Detección de anomalías", "NLP"],
        },
        {
          num: "05",
          icon: "Database",
          title: "Datos limpios y automatización",
          desc: "Procesos que hoy hace una persona a mano, convertidos en automáticos. Pipelines ETL que integran sistemas y mantienen los datos limpios y confiables.",
          modules: ["Python", "Pandas", "ETL", "SQL avanzado", "Calidad del dato", "Integración de fuentes"],
        },
        {
          num: "06",
          icon: "Server",
          title: "Proyectos de gran volumen",
          desc: "Cuando los datos superan lo que Excel puede manejar. Arquitecturas robustas para procesar millones de registros sin perder nada en el camino.",
          modules: ["Big Data", "Apache Spark", "NoSQL", "Pipelines de datos", "Procesamiento distribuido"],
        },
      ],
    },
    blog: {
      tag: "Blog",
      title: "Ideas para usar mejor tus datos",
      subtitle: "Sin jerga técnica. Casos reales, herramientas prácticas y una sola idea por artículo.",
      readMore: "Leer artículo",
      readTime: "min de lectura",
      backToHome: "← Volver al inicio",
      backToBlog: "← Volver al blog",
      allArticles: "Ver todos los artículos",
    },
    footer: {
      tagline: "Analista de Datos · Business Intelligence · Data Scientist · Data Engineer",
      location: "Resistencia, Chaco, Argentina",
      links: {
        services: "Servicios",
        howIWork: "Cómo trabajo",
        projects: "Proyectos",
        about: "Sobre mí",
        blog: "Blog",
        contact: "Contacto",
      },
      copy: "Cristian Moreira. Todos los derechos reservados.",
    },
  },
  en: {
    nav: {
      services: "Services",
      howIWork: "How I work",
      projects: "Projects",
      about: "About",
      contact: "Contact",
      blog: "Blog",
      cta: "Let's talk",
    },
    hero: {
      badge: "Data Analyst · Business Intelligence",
      headline1: "Your data already exists.",
      headline2: "What's missing is turning it into information.",
      sub: "Every sale, every purchase, every move your business makes leaves a trace. I read that trace and tell you what to do with it.",
      cta1: "Let's talk about your business",
      cta2: "See services",
      stat1: { value: "12+", label: "years in retail & consumer goods" },
      stat2: { value: "4", label: "leading companies analyzed" },
      stat3: { value: "100%", label: "guaranteed confidentiality" },
    },
    problem: {
      tag: "The diagnosis",
      title: "You have data. What you're missing is information.",
      subtitle:
        "Dates, numbers, sales, transactions — it's all stored somewhere. The problem isn't the volume: it's that without proper analysis, data says nothing. Information does.",
      cards: [
        {
          icon: "TrendingDown",
          title: "Prices bleeding margin",
          desc: "You sell a lot but earn little. Without cost and margin analysis, it's impossible to know where you're losing profitability.",
        },
        {
          icon: "Package",
          title: "Inventory that sleeps or runs out",
          desc: "Idle stock that costs money, or stockouts that lose you sales. Both have solutions with data.",
        },
        {
          icon: "BarChart2",
          title: "KPIs no one understands",
          desc: "Reports full of numbers without context. You know something is wrong, but not exactly what or why.",
        },
        {
          icon: "Clock",
          title: "Time lost in spreadsheets",
          desc: "Hours building reports manually that could be automated. That time is valuable — and it can be recovered.",
        },
      ],
    },
    services: {
      tag: "What I do",
      title: "From data to actionable information",
      subtitle:
        "I read what you already record and deliver concrete information: what's happening and why. The decision is yours.",
      items: [
        {
          icon: "DollarSign",
          title: "Pricing & Margin Analysis",
          desc: "Cost models, product profitability, and promotional simulation so every commercial decision is backed by evidence.",
          tags: ["Advanced Excel", "Power BI"],
        },
        {
          icon: "LayoutDashboard",
          title: "Dashboards & KPIs",
          desc: "Visual boards showing what really matters in your business — updated and easy to read.",
          tags: ["Power BI", "Looker Studio"],
        },
        {
          icon: "RefreshCw",
          title: "Report Automation",
          desc: "Turn hours of manual work into automated processes. More time for your business, less for spreadsheets.",
          tags: ["Python", "Power Query"],
        },
        {
          icon: "Package",
          title: "Inventory Analysis",
          desc: "Product rotation, expiry dates, optimal stock. Make purchasing decisions with evidence, not intuition.",
          tags: ["SQL", "Excel"],
        },
        {
          icon: "Database",
          title: "Data Organization",
          desc: "Your scattered data, clean and structured. A solid foundation for any future analysis.",
          tags: ["SQL", "Python", "ETL"],
        },
        {
          icon: "MessageSquare",
          title: "Custom Consulting",
          desc: "If your problem doesn't fit a standard box, we work together from diagnosis to solution.",
          tags: ["Custom"],
        },
      ],
    },
    howIWork: {
      tag: "How I work",
      title: "Your data speaks. I interpret it.",
      subtitle: "I'm not an agency. I'm a committed data analyst focused on your business growth.",
      steps: [
        {
          num: "01",
          title: "I listen before I analyze",
          desc: "First I understand your business, your decisions, and your pain points. The data comes after.",
        },
        {
          num: "02",
          title: "I work with what you have",
          desc: "You don't need expensive systems or big changes. I start with your spreadsheets, your system, and your current data.",
        },
        {
          num: "03",
          title: "I explain without jargon",
          desc: "Every analysis comes with a clear conclusion: what it means, what you can do, and what the impact is.",
        },
        {
          num: "04",
          title: "I'm available when you need me",
          desc: "I don't deliver a report and disappear. I'm your ongoing support for interpreting what's happening in your business.",
        },
      ],
      values: [
        { icon: "Shield", label: "Full confidentiality" },
        { icon: "Handshake", label: "Real commitment" },
        { icon: "Clock", label: "Availability & response" },
        { icon: "Heart", label: "Service vocation" },
      ],
    },
    projects: {
      tag: "Projects",
      title: "Real analyses, concrete results",
      subtitle: "A sample of work where data generated real value.",
      items: [
        {
          title: "Cost & Commercial Pricing Analysis",
          company: "Leading consumer goods company · 2026",
          desc: "Advanced Excel models to evaluate cost structure, margins, and promotional actions. Power BI KPIs and dashboards for commercial performance tracking.",
          tags: ["Advanced Excel", "Power BI", "Pricing"],
          highlight: "Better margin visibility by category",
        },
        {
          title: "Internet Access Analysis by Province",
          company: "Henry Bootcamp · 2024",
          desc: "EDA and cleanup of national connectivity datasets. Interactive Power BI dashboards to identify gaps and expansion opportunities by province.",
          tags: ["Python", "Pandas", "Power BI"],
          highlight: "8 provinces with critical gap identified",
          link: "https://github.com/moreiracristian/Henry-PIDA-CM",
        },
        {
          title: "MVP for Uber — Environmental Analysis",
          company: "Henry Bootcamp · 2024",
          desc: "Analysis of environmental variables impacting Uber operations in New York. Electric vehicle adoption proposal with complete ETL pipeline.",
          tags: ["Python", "Airflow", "MySQL", "Streamlit"],
          highlight: "40% projected reduction in operating costs",
          link: "https://github.com/moreiracristian/Henry-Arcope-PF",
        },
        {
          title: "Movie Recommendation System",
          company: "Henry Bootcamp · 2024",
          desc: "Content similarity recommendation engine with a FastAPI endpoint for real-time queries. Machine learning applied to real IMDB data.",
          tags: ["Scikit-learn", "FastAPI", "Python"],
          highlight: "REST API with real-time response",
          link: "https://github.com/moreiracristian/Henry-PIMLOps-CM",
        },
      ],
    },
    about: {
      tag: "About",
      title: "12 years reading numbers that move businesses",
      paragraphs: [
        "I worked for over a decade at the largest retail companies in Argentina — Falabella, Cencosud, Sodimac, Cetrogar. I witnessed firsthand how data-driven decisions change business outcomes — and how the lack of analysis sinks them.",
        "In those years I learned that data problems are always business problems: prices that erode margin without anyone noticing, inventory frozen in the warehouse, reports that get built but nobody reads. I understand that language and know how to turn it into concrete decisions.",
        "Today I support SMEs that want to grow but don't have their own data analyst. I'm your data backbone: committed, available, and always in business language.",
        "Trained in Data Science (Henry Bootcamp, +800 hours), Business Intelligence (UTN), and Systems Engineering. All in service of a simple goal: making your data work for you.",
      ],
      skills: [
        "Advanced Excel",
        "Power BI",
        "SQL",
        "Python",
        "Power Query",
        "Tableau",
        "Airflow",
        "PostgreSQL",
      ],
      experience: [
        { company: "Leading consumer goods company", role: "Purchasing Analyst", period: "2025 – Present" },
        { company: "CETROGAR S.A.", role: "Collections Analyst", period: "2019 – 2025" },
        { company: "CENCOSUD S.A.", role: "Admin. & Management Control Analyst", period: "2013 – 2016" },
        { company: "FALABELLA S.A.", role: "Corporate Treasury Analyst", period: "2010 – 2013" },
      ],
    },
    contact: {
      tag: "Contact",
      title: "Tell me about your business",
      subtitle: "All inquiries are confidential. No commitment. Reply within 24 hours.",
      form: {
        name: "Your name",
        email: "Your email",
        company: "Your company / industry",
        message: "What's your biggest challenge with your business data?",
        send: "Send message",
        sending: "Sending...",
        success: "Message received! I'll reply within 24 hours.",
        error: "Something went wrong. Email me directly at moreiracristian@2moreit.com",
      },
      direct: {
        title: "Or reach out directly",
        whatsapp: "WhatsApp",
        email: "Email",
        linkedin: "LinkedIn",
      },
    },
    newsletter: {
      tag: "Newsletter",
      title: "One business insight per week",
      subtitle:
        "No spam. No technical jargon. Just one practical idea each week to help you use your data better.",
      placeholder: "Your email",
      cta: "Subscribe",
      subscribing: "Subscribing...",
      success: "Subscribed! Your first insight is coming this week.",
      error: "Something went wrong. Please try again.",
      privacy: "No spam. Unsubscribe anytime.",
    },
    segments: {
      tag: "Who I work with",
      title: "Your business has data. I turn it into information.",
      subtitle: "I work with companies that want to grow but don't have their own data analyst.",
      items: [
        {
          icon: "ShoppingCart",
          title: "Retail & consumer goods",
          desc: "Companies with products, sales, and costs that need visibility into margins, turnover, and commercial performance.",
          examples: ["Supermarkets", "Distributors", "Importers"],
        },
        {
          icon: "Truck",
          title: "Wholesalers & distributors",
          desc: "High-turnover businesses that need to control replenishment, stockouts, and profitability by customer.",
          examples: ["Wholesalers", "Logistics", "B2B suppliers"],
        },
        {
          icon: "Store",
          title: "Shops & stores",
          desc: "Retailers that want to know which products to sell, when to restock, and how to maximize margin per square meter.",
          examples: ["Hardware stores", "Pharmacies", "Apparel"],
        },
        {
          icon: "Building2",
          title: "Companies without a data team",
          desc: "Any SME making important decisions with outdated spreadsheets or without clear metrics.",
          examples: ["Services", "Manufacturing", "Agribusiness"],
        },
      ],
    },
    faq: {
      tag: "FAQ",
      title: "What people ask me before we start",
      subtitle: "If your question isn't here, reach out. Every inquiry is welcome.",
      items: [
        {
          q: "Do I need a special system to work with you?",
          a: "No. I work with what you already have: Excel, Google Sheets, exports from your management system, or even manual files. You don't need to invest in new technology to get started.",
        },
        {
          q: "How long does it take to get the first results?",
          a: "It depends on the type of analysis, but generally the first report is ready within 5 to 10 business days of receiving the data. Power BI dashboards may take a bit longer depending on complexity.",
        },
        {
          q: "Is my data safe and confidential?",
          a: "Yes, completely. We sign a confidentiality agreement before sharing any files. I never share or use your data for any other purpose. Your information is yours.",
        },
        {
          q: "What's the difference between standard services and more complex projects?",
          a: "Standard services solve visibility problems: dashboards, margin analysis, report automation — you see what you already have, more clearly. More complex projects go a step further: full process automation, models that predict future behavior, or infrastructure to handle large data volumes. The difference isn't the tool — it's the depth and scope of the problem being solved.",
        },
        {
          q: "What kind of company is a good candidate for a more complex project?",
          a: "Companies that already have accumulated historical data, a significant volume of operations, and specific questions that a standard dashboard can't answer. You don't need your own IT team — you need a clear problem and available data. In the first conversation we'll evaluate together whether it makes sense to take it on.",
        },
        {
          q: "What if I don't know exactly what I need?",
          a: "That's exactly what the first conversation is for. Tell me how you make decisions today, what information you're missing, and what problem keeps you up at night. From there we define together what makes sense to analyze.",
        },
        {
          q: "What size companies do you work with?",
          a: "Companies of different sizes and needs. From SMEs just starting to organize their data, to mid-sized businesses with more complex operations that need automation, predictive models, or data infrastructure at scale. Size doesn't define whether I can help — the type of problem you have does.",
        },
      ],
    },
    techExpertise: {
      tag: "For more complex projects",
      title: "When the business needs more than a report",
      subtitle: "For companies with more data, more systems, or more unanswered questions. Same commitment, greater depth.",
      cta: "Let's talk about your project",
      tracks: [
        {
          num: "01",
          icon: "BarChart3",
          title: "Business Intelligence & Dashboards",
          desc: "Consolidation of multiple data sources into visual, interactive panels that display business performance in real time.",
          modules: ["Power BI", "Tableau", "Advanced DAX", "Power Query", "Matplotlib / Seaborn"],
        },
        {
          num: "02",
          icon: "LineChart",
          title: "People Analytics",
          desc: "Data analytics applied to HR: hiring metrics, regional labor market analysis, and turnover evaluation to optimize talent retention.",
          modules: ["SQL", "Python", "HR KPIs", "Descriptive statistics", "Data visualization"],
        },
        {
          num: "03",
          icon: "Code2",
          title: "Competitive Intelligence",
          desc: "Automated market and competitor monitoring — price tracking, offers, and bids — to dynamically adjust commercial strategies.",
          modules: ["Web scraping", "Python", "Automation", "SQL", "Automated reports"],
        },
        {
          num: "04",
          icon: "Brain",
          title: "Predictive Models",
          desc: "Your business anticipates instead of reacting. Demand forecasting, churn detection, and future sales estimation using historical data.",
          modules: ["Demand forecasting", "Classification", "Time series", "Anomaly detection", "NLP"],
        },
        {
          num: "05",
          icon: "Database",
          title: "Clean data & automation",
          desc: "Processes done manually today, converted into automatic ones. ETL pipelines that integrate systems and keep data clean and reliable.",
          modules: ["Python", "Pandas", "ETL", "Advanced SQL", "Data quality", "Source integration"],
        },
        {
          num: "06",
          icon: "Server",
          title: "Large-scale projects",
          desc: "When data exceeds what Excel can handle. Robust architectures to process millions of records without losing anything along the way.",
          modules: ["Big Data", "Apache Spark", "NoSQL", "Data pipelines", "Distributed processing"],
        },
      ],
    },
    blog: {
      tag: "Blog",
      title: "Ideas to use your data better",
      subtitle: "No technical jargon. Real cases, practical tools, and one idea per article.",
      readMore: "Read article",
      readTime: "min read",
      backToHome: "← Back to home",
      backToBlog: "← Back to blog",
      allArticles: "View all articles",
    },
    footer: {
      tagline: "Data Analyst · Business Intelligence · Data Scientist · Data Engineer",
      location: "Resistencia, Chaco, Argentina",
      links: {
        services: "Services",
        howIWork: "How I work",
        projects: "Projects",
        about: "About",
        blog: "Blog",
        contact: "Contact",
      },
      copy: "Cristian Moreira. All rights reserved.",
    },
  },
};
