        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: "1.5rem" }}>
          {s.items.map((item) => {
            ...
            return (
              <div
                key={item.title}
                style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "1rem", padding: "1.75rem", display: "flex", flexDirection: "column", gap: "1rem", transition: "border-color 0.2s, transform 0.2s" }}
              ...
              >
                <div style={{ width: "3rem", height: "3rem", borderRadius: "0.75rem", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg, var(--accent-cyan-dim), var(--accent-blue-dim))", border: "1px solid var(--border)", flexShrink: 0 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: "0.4rem", marginTop: "auto" }}>
                  {item.examples.map((ex) => (
                    <span key={ex} style={{ fontSize: "0.7rem", padding: "0.2rem 0.6rem", borderRadius: "0.375rem", background: "rgba(6,182,212,0.1)", color: "var(--accent-cyan)", border: "1px solid rgba(6,182,212,0.2)" }}>
                      {ex}
                    </span>
                  ))}
                </div>
